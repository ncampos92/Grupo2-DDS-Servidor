class ProplikesController < ApplicationController
  ...
  before_action :set_proplike_by_user, only: :delete_by_user
  ...
  private
    def authorize
      respond_to do |format|
        format.html{
          if current_user && @proplike && (@proplike.user.id == current_user.id || is_admin?)
          else
        ...
        format.json{
          if current_user_api && @proplike && (@proplike.user.id == current_user_api.id || is_admin?)
          else
            render json: {
              error: "Denied Acces",
              status: :forbidden
    ...
    def set_proplike_by_user
      @proplike = @proposal.proplikes.find_by(user_id: current_user_api)
    end
